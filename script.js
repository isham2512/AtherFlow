...
class AetherFlow {
  ...
  bindEvents() {
    ...
    const volRow = this.d.volSlider.closest('.vol-row');
    if (volRow) {
      volRow.addEventListener('click', e => {
        e.stopPropagation();
        volRow.classList.add('revealed');
      });
    }

    this.d.btnMute.addEventListener('click', e => {
      const volRow = this.d.volSlider.closest('.vol-row');
      if (volRow) {
        const isSliderHidden = window.getComputedStyle(this.d.volSlider).opacity === '0';
        if (isSliderHidden) {
          volRow.classList.add('revealed');
          e.stopPropagation();
          e.preventDefault();
    ...
    document.addEventListener('click', () => {
      const volRow = this.d.volSlider.closest('.vol-row');
      if (volRow) {
        volRow.classList.remove('revealed');
      }
    });
